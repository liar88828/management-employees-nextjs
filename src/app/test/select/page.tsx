...
function Page() {
	...
	const handleEncryptChange = async (e: ChangeEvent<HTMLInputElement>) => {
		if (e.target.files && e.target.files.length > 0) {
			setOriginalImage(URL.createObjectURL(e.target.files[0]));
			await uploadAndEncrypt(e.target.files[0]);
		}
	};

	const handleDecryptChange = async (e: ChangeEvent<HTMLInputElement>) => {
		if (e.target.files && e.target.files.length > 0) {
			await uploadAndDecrypt(e.target.files[0]);
		}
	};
