...
import { EmployeeUserPhotoClient } from "@/interface/model";


export function AdminRegistrationPage({ employees, name, status, page, totalPages }: {
	name: string,
	status: string,
	employees: EmployeeUserPhotoClient[],
	page: number,
	totalPages: number
}) {
	return (
			...
			<div className="overflow-x-auto w-full">
				...
				>
					...
					{ employees.map((employee, index) => {
						...
						const imageEmployee = (
							employee?.ImageEmployee?.photoKtp === null ||
							employee?.ImageEmployee?.photoIjazah === null
						)
							? 'Not Complete'
							: 'Complete';
						...
						return (
							<tr key={ employee.id } className="hover:bg-gray-100/20">
								...
								<td>
									...
									<p className={ ' badge badge-info  text-nowrap' }>{ imageEmployee }</p>
								</td>
								<td>
									<div>
										<Link
